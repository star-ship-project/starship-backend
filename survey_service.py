class SurveyService:
    ...
    def process_sms(self, phone: str, text: str):
        ...
        user_id, deped_id, school_id, first_name, middle_name, last_name, suffix_name, sex, age, phone_number, step, errors = user_row
        ...
        if step == 1:
            self.db.update_bio(user_id, "deped_id", user_text, 2)
        ...
        if is_number:
            if step == 4:
                self.db.update_bio(user_id, "age", value, 5)
                print(f"[DB] Saved age = {value} for user_id {user_id}")
                self.sms.send_sms(phone, self.questions[5])
                return
            elif step == 6:
                self.db.update_professional(deped_id, "years_experience", value)
                self.db.update_step(user_id, 7)
                print(f"[DB] Saved years_experience = {value} for user_id {user_id}")
                self.sms.send_sms(phone, self.questions[7])
                return
            elif step == 10:
                self.db.update_professional(deped_id, "device_count", value)
                self.db.update_step(user_id, 11)
                print(f"[DB] Saved device_count = {value} for user_id {user_id}")
            ...
            if step == 2:
                self.db.update_bio(user_id, "school_id", user_text, 3)
                print(f"[DB] Saved school_id = {user_text} for user_id {user_id}")
                self.sms.send_sms(phone, self.questions[3])
                return
            elif step == 5:
                self.db.update_bio(user_id, "sex", user_text, 6)
                print(f"[DB] Saved sex = {user_text} for user_id {user_id}")
                self.db.create_professional_record(user_id, deped_id)
                self.sms.send_sms(phone, self.questions[6])
                return
            elif step == 7:
                self.db.update_professional(deped_id, "role_position", user_text)
                self.db.update_step(user_id, 8)
                print(f"[DB] Saved role_position = {user_text} for user_id {user_id}")
                self.sms.send_sms(phone, self.questions[8])
                return
            elif step == 8:
                self.db.update_professional(deped_id, "specialization", user_text)
                self.db.update_step(user_id, 9)
                print(f"[DB] Saved specialization = {user_text} for user_id {user_id}")
                ...
                if user_lower in ["yes", "oo", "oo naman", "yes po"]:
                    ...
                    if errors < 1:
                        self.db.increment_error(user_id)
                        self.sms.send_sms(phone, "Pakisagot lang ng 'yes' o 'no': ")
                    else:
                        print(f"[SKIP] Error limit reached for user_id {user_id}, not sending.")
                    return
                self.db.update_professional(deped_id, "is_internet_access", is_internet)
                self.db.update_step(user_id, 10)
                print(f"[DB] Saved is_internet_access = {is_internet} for user_id {user_id}")
